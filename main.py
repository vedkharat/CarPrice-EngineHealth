...
PRICE_MODEL_URL = "https://drive.google.com/uc?export=download&id=1cfNPQwWBjt_QXYVFE_7pUa-rahi8yVEi"
...
def download_if_missing(url: str, dest_path: Path) -> None:
    ...
    Uses gdown so that large files (which trigger the 'virus scan' warning page)
    are downloaded correctly instead of saving the HTML page.
    """
    if dest_path.exists():
        # Already downloaded in this container
        return

    with st.spinner("Downloading price model bundle from Google Drive…"):
        try:
            # gdown understands normal 'file/d/<id>/view' links
            gdown.download(url, str(dest_path), quiet=False, fuzzy=True)

            if not dest_path.exists() or dest_path.stat().st_size < 1024:
                # File is suspiciously small – treat as failure
                raise RuntimeError(
                    f"Downloaded file at {dest_path} looks invalid (too small)."
                )

        except Exception as e:
            st.error(
                "Could not download the price model bundle from Google Drive. "
                "Please try again later."
            )
            # Re-raise so Streamlit shows the traceback in the logs
            raise
